...
from config.config import (
    PROJECT_ID, ARTIFACTS_GCS_PREFIX,
...
def _download_preprocessor(local_path: str):
    ...
    if not blob.exists():
        raise SystemExit("❌ Preprocessor not found in GCS. Re-run training to save it (pipeline saves it).")
    blob.download_to_filename(local_path)

def main(n=1, out="instances.json"):
    # 1) Load curated df the same way as the pipeline
    df = load_curated()
    if df.empty:
    ...
    # 2) Build the same tabular column set the model expects
    ...
    # 3) Load preprocessor and transform N rows
